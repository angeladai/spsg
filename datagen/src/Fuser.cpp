	...
	const std::vector<unsigned int>& incompleteFrames, bool debugOut /*= false*/)
{
	...
	const bool bGenerateColors = gas.s_bRenderColors;
	...
	ColorImageR8G8B8 color; if (bGenerateColors) color.allocate(imageWidth, imageHeight);
	...
	for (unsigned int i = 0; i < initialFramesToScan.size(); i++) {
		bool bValid = scene.getDepthFrame(m_app.graphics, initialFramesToScan[i], rawDepth, color, intrinsic, extrinsic);
		if (bValid) {
			...
			grid.integrate(intrinsic, extrinsic, filtDepth, color);
		}
		std::cout << "\r[ " << i << " | " << initialFramesToScan.size() << " ]";
	}
	if (bGenerateSdfs && !outputIncompleteFile.empty()) { // save incomplete to file
		grid.saveToFile(outputIncompleteFile, bSaveSparse, bGenerateColors, saveSparseTruncFactor);
		...
		if (debugOut) {
			...
			re.loadFromFile(outputIncompleteFile, bSaveSparse, bGenerateColors);
			...
			for (const auto& v : grid) {
				...
				if (bGenerateColors && rv.color != v.value.color) {
					std::cout << "error (incomplete) load/save voxel mask " << (int)v.value.color.b << " vs " << (int)rv.color.b << std::endl;
					getchar();
				}
			}
			...
			{ // marching cubes
				...
				MeshDataf meshMC = MarchingCubes::doMC(copy, 10.0f, bGenerateColors);
	...
	if (bGenerateComplete) { // scan complete
		for (unsigned int i = 0; i < restOfFramesToScan.size(); i++) {
			bool bValid = scene.getDepthFrame(m_app.graphics, restOfFramesToScan[i], rawDepth, color, intrinsic, extrinsic);
			if (bValid) {
				// filter the depth map
				if (bFilterDepth) CameraUtil::bilateralFilter(rawDepth, 2.0f, 0.1f, filtDepth);
				else std::swap(rawDepth, filtDepth);
		...
		if (bGenerateSdfs) grid.saveToFile(outputCompleteFile, bSaveSparse, bGenerateColors, saveSparseTruncFactor);
		...
		if (debugOut) {
			...
			VoxelGrid re(vec3ui(0, 0, 0), mat4f::identity(), 1.0f, OBB3f(), 0.1f, 10.0f);
			re.loadFromFile(outputCompleteFile, bSaveSparse, false);
			...
			{ // marching cubes
				VoxelGrid copy(grid); copy.normalizeSDFs(); copy.setWorldToGrid(mat4f::identity()); //copy.improveSDF(1);
				MeshDataf meshMC = MarchingCubes::doMC(copy, 10.0f, bGenerateColors);
