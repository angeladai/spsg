...
public:
	...
	void loadMatterport(std::string scanPath, const std::string& sceneListFile, unsigned int maxNumSens) {
		if (scanPath.back() != '/' && scanPath.back() != '\\') scanPath.push_back('/');

		const std::string meshExt = ".reduced.ply"; // the same as the .ply but without any face attributes
		...
		while (std::getline(s, room)) {
			...
			const std::string meshFile = scanPath + "/" + scene + "/" + dataSubPath + "/region" + roomId + meshExt;
			const std::string aggFile = scanPath + "/" + scene + "/" + dataSubPath + "/region" + roomId + aggExt;
			const std::string segFile = scanPath + "/" + scene + "/" + dataSubPath + "/region" + roomId + segExt;
