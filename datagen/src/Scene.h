...
public:
	...
	void load(GraphicsDevice& g, const ScanInfo& scanInfo, bool bUseRenderedDepth, const std::string& trajCacheFile = "", mat4f transform = mat4f::identity());
	void updateRoom(GraphicsDevice& g, const ScanInfo& scanInfo, bool bUseRenderedDepth, const std::string& trajCacheFile = "", mat4f transform = mat4f::identity());
	...
	bool getDepthFrame(GraphicsDevice& g, unsigned int idx, DepthImage32& depth, ColorImageR8G8B8& color, mat4f& intrinsic, mat4f& extrinsic, float minDepth = 0.1f, float maxDepth = 12.0f);
	bool getRawDepthFrame(unsigned int idx, DepthImage32& depth, ColorImageR8G8B8& color, mat4f& intrinsic, mat4f& extrinsic, float minDepth = 0.1f, float maxDepth = 12.0f) const;
	bool renderDepthFrame(GraphicsDevice& g, unsigned int idx, DepthImage32& depth, ColorImageR8G8B8& color, mat4f& intrinsic, mat4f& extrinsic, float minDepth = 0.1f, float maxDepth = 12.0f);
	...
	void computeTrajFramesInScene(std::vector<unsigned int>& frameIds) const {
		...
		//}
		//MeshIOf::saveToFile("_cam_in_scene.ply", makeCamerasMesh(camerasInScene, 1, vec4f(0.0f, 1.0f, 0.0f, 1.0f)));
		//MeshIOf::saveToFile("_cam_not_in_scene.ply", makeCamerasMesh(camerasNotInScene, 10, vec4f(1.0f, 1.0f, 0.0f, 1.0f)));
